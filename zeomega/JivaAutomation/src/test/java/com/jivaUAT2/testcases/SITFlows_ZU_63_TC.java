...
public class SITFlows_ZU_63_TC extends TestBase {
	private static Logger logger = Logger.getLogger(SITFlows_ZU_63_TC.class);
	...
	public void verify_MemberData_fromfile_toScreen_ZU_60Integrationflow() throws InterruptedException {
		...
		addInteractionsPage.add1stInteractionforCCM(sUSERPROFILENAME);
		...
		// ----- Verify Post Enrollment letter generation -----	
		episodeoverviewpage.clickHamBurger();
		episodeoverviewpage.clickCorrespondence();
		Assert.assertEquals(true, episodeoverviewpage.verify_CM_PostEnrollmentLetterGenerated(sUSERPROFILENAME),
		...
		logger.info("Successfully completed validating integration flow of #ZU-63_CCM_Member_Identified Needs/Goals Have Been Met# for the member holding the Member Id: "+MemberDemographicData.get(ALTERNATEID)+" and for the client: "+clientname);
		
		Reporter.log("Successfully completed validating integration flow of #ZU-63_CCM_Member_Identified Needs/Goals Have Been Met# for the member holding the Member Id: "+MemberDemographicData.get(ALTERNATEID)+" and for the client: "+clientname);
